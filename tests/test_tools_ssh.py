...
def test_parse_agent_keys_l_format() -> None:
    ...
    text = "256 SHA256:abcdef123 user@host (ED25519)\n3072 SHA256:fedcba987 (RSA)\n"
...
def test_keys_list_json_empty(
    ...
    result = runner.invoke(app, ["ssh", "keys", "list", "--ssh-dir", str(tmp_path), "--json"])
...
def test_keys_list_json_with_pair(
    ...
    (tmp_path / "id_ed25519.pub").write_text("ssh-ed25519 AAAA-pub u@h\n")
    result = runner.invoke(app, ["ssh", "keys", "list", "--ssh-dir", str(tmp_path), "--json"])
    assert result.exit_code == 0
    doc = json.loads(result.stdout)
...
def test_keys_generate_invokes_ssh_keygen(
    ...
    monkeypatch.setattr("shimkit.tools.ssh.manager.CommandRunner.run", staticmethod(fake_run))
...
def test_keys_generate_refuses_overwrite(
    ...
    _force_linux(monkeypatch)
    (tmp_path / "id_test").write_text("-----BEGIN OPENSSH PRIVATE KEY-----\n")
    result = runner.invoke(
        app,
...
def test_agent_status_reports_no_agent(runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
    _force_linux(monkeypatch)
    monkeypatch.setattr(
        "shimkit.tools.ssh.manager.CommandRunner.run",
        staticmethod(lambda _cmd, **_: CommandResult(2, "", "Could not open a connection.")),
    )
...
def test_agent_status_reports_loaded_keys(
    ...
    monkeypatch.setattr(
        "shimkit.tools.ssh.manager.CommandRunner.run",
        staticmethod(
            lambda _cmd, **_: CommandResult(0, "256 SHA256:fp1 user@host (ED25519)\n", "")
        ),
    )
    result = runner.invoke(app, ["ssh", "agent", "status", "--json"])
    assert result.exit_code == 0
...
def test_known_hosts_audit_clean(
    ...
    _force_linux(monkeypatch)
    (tmp_path / "known_hosts").write_text("host1 ssh-ed25519 AAA1\nhost2 ssh-rsa BBB2\n")
    result = runner.invoke(
        app,
...
def test_known_hosts_prune_removes_duplicates(
    ...
    kh.write_text("host1 ssh-ed25519 AAA1\nhost1 ssh-ed25519 AAA1\nhost2 ssh-rsa BBB2\n")
    result = runner.invoke(
        app,
...
def test_perms_audit_flags_loose_key(
    ...
    priv.write_text("-----BEGIN OPENSSH PRIVATE KEY-----\n")
    priv.chmod(0o644)
    result = runner.invoke(app, ["ssh", "perms", "audit", "--ssh-dir", str(tmp_path), "--json"])
    assert result.exit_code == 0
    doc = json.loads(result.stdout)
    assert any(Path(i["path"]) == priv and i["expected"] == "600" for i in doc["data"]["issues"])
...
def test_perms_fix_chmods_offenders(
    ...
    priv.write_text("-----BEGIN OPENSSH PRIVATE KEY-----\n")
    priv.chmod(0o644)
    result = runner.invoke(app, ["ssh", "perms", "fix", "--ssh-dir", str(tmp_path), "--yes"])
    assert result.exit_code == 0
